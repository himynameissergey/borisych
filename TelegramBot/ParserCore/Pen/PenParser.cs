...
namespace TelegramBot.ParserCore.Pen
{
    class PenParser : IParser<string[]>
    {
        public string[] Parse(IHtmlDocument document)
        {
            ...
            var items = document.QuerySelectorAll("a").Where(item => item.Attributes["href"] != null && item.Attributes["href"].Value.Contains("http://penitto.ru/?p=")).OfType<IHtmlAnchorElement>(); //penitto.ru
